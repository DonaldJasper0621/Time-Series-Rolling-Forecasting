...
library(quadprog)
library(fTrading)
source("C:/Users/88690/Desktop/R_Homework/function_FDA.R")
...
#???????????????????????????
rf<-read_excel("C:/Users/88690/Desktop/R_Homework/RiskFree.xlsx")
...
df_summary<-apply(df_ret_per[,2:21],2,function(x){
  c(n=length(x),
    ...
    mean=mean(x*250),
    sd=sd(x)*sqrt(250),
    skewness=my_skewness(x*250),
    kurtosis=my_kurtosis(x*250),
    acf1=my_acf1(x),
    VaR=VaR_samplex(x,1,0.05),
    ES=ES_samplex(x,1,0.05),
    LPSD=LPSDx(x,rf$rf),
    MaxDrawdown=maxDrawdown(x/100))
})
...
weight<-mvp_wx_quad(df_train[,-1],2/250)$solution
...
df_summary<-apply(portfolio_out[,2:8],2,function(x){
  c(n=length(x),
    ...
    acf1=my_acf1(x),
    rf=tail(rf$rf,1),
    Sharp=(mean(x)-tail(rf$rf,1)/250)/sd(x),
    VaR=VaR_samplex(x,1,0.05),
    ES=ES_samplex(x,1,0.05),
...
turnover<-apply(weight,2,function(x){
  ifelse(min(sum(x[which(x>0)]),abs(sum(x[which(x<0)])))/abs(sum(x))==0,1,min(sum(x[which(x>0)]),abs(sum(x[which(x<0)])))/abs(sum(x)))
})
